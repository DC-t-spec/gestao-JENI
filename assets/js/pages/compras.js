...
export async function renderPurchases() {
  ...
  const { data: rows } = await supabase.from('purchases').select('*').order('purchase_date', { ascending: false }).limit(20);
    ...
    <div class="split">
      <div class="card">
        ...
        <form id="purchase-form" class="form-grid">
          <div class="field"><label>Data da compra</label><input type="date" name="purchase_date" required /></div>
          <div class="field"><label>Lote</label><select name="batch_id">${batchOptions()}</select></div>
          <div class="field"><label>Item comprado</label><input type="text" name="item_name" required /></div>
          <div class="field"><label>Categoria</label><input type="text" name="category" required placeholder="Ex: ração, vacina, transporte" /></div>
          <div class="field"><label>Quantidade</label><input type="number" step="0.01" name="quantity" required /></div>
          <div class="field"><label>Unidade</label><input type="text" name="unit" placeholder="kg, saco, unidade" /></div>
          ...
          <div class="field"><label>Onde comprou</label><input type="text" name="purchase_place" /></div>
          <div class="field full"><label>Observações</label><textarea name="notes"></textarea></div>
          <div class="field full"><button class="btn btn-primary" type="submit">Guardar compra</button></div>
        </form>
      ...
      <div class="card">
        ...
        <div class="table-wrap">
          <table>
            ...
            <tbody>
              ${(rows || []).map((row) => `
                <tr><td>${row.purchase_date || ''}</td><td>${row.item_name || ''}</td><td>${row.category || ''}</td><td>${row.quantity || ''}</td><td>${formatMoney(row.total_amount)}</td></tr>
              `).join('') || '<tr><td colspan="5">Sem compras registadas.</td></tr>'}
            </tbody>
          </table>
        </div>
      </div>
